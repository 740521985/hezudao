...
(function() {
    ...
    function e(H) {
        ...
        closeWin = function(J, I) {
            unLoadGrayBackground();
            ableClickSubmitButtonOrPreStepBUtton();
            if (dhxWins.isWindow(J + "_")) {
                dhxWins.window(J + "_").setModal(false);
                dhxWins.window(J + "_").hide();
                if (I) {
        ...
        ;
        b = function(P, M, J, I, L) {
            var O = '<div class="tit">' + (M ? '<span class="colorC">' + P + "</span>" : P) + "</div>";
            var K = "<P>" + J + "</p>";
            var N = M ? '<p>请点击[<a href="' + ctx + 'queryOrder/init"><strong>我的12306</strong></a>]办理其他业务。您也可以点击[<a href="' + ctx + 'leftTicket/init"><strong>预订车票</strong></a>]，重新规划您的旅程。</p>' : '<P>查看订单处理情况，请点击“<a href="' + ctx + 'queryOrder/initNoComplete">未完成订单</a>”</P>';
            $("#iamge_status_id").removeClass().addClass("icon i-" + L);
            if (I) {
            ...
            $("#orderResultInfo_id").html(O + (J == "" || J == null  || J == "undefined" || J == undefined ? "" : K) + N);
            A("transforNotice_id");
            if (I) {
        ...
        function A(M) {
            ...
            if ("checkticketinfo_id" == M) {
                var K = ticketInfoForPassengerForm.queryLeftNewDetailDTO;
                if (K.to_station_telecode == ticket_submit_order.special_areas.lso || K.to_station_telecode == ticket_submit_order.special_areas.dao || K.to_station_telecode == ticket_submit_order.special_areas.ado || K.to_station_telecode == ticket_submit_order.special_areas.nqo || K.to_station_telecode == ticket_submit_order.special_areas.tho) {
                    if (B()) {
            ...
            var J = j(M);
            var I = $(window).width() / 2 - 300;
            var L = o() + ($(window).height() / 2 - 200);
            J.setDimension($("#content_" + M).width(), $("#content_" + M).height() + 10);
            $(".dhtmlx_window_active").height($("#content_" + M).height());
        ...
        function u() {
            ...
            for (var J = 0; J < limit_tickets.length; J++) {
                var I = limit_tickets[J];
                if (!(I.seat_type == "F" || I.seat_type == "A")) {
                    return false
                }
            }
            return true
        ...
        function l() {
            for (var J = 0; J < limit_tickets.length; J++) {
                var I = limit_tickets[J];
                if ((I.ticket_type == ticket_submit_order.ticket_type.child || I.ticket_type == ticket_submit_order.ticket_type.adult) && I.id_type == "1" && (I.seat_type == "F" || I.seat_type == "A")) {
                    if (Number(I.id_no.substr(17, 18)) % 2 != 0) {
                        return false
                    }
                } else {
                    return false
        ...
        function n() {
            for (var J = 0; J < limit_tickets.length; J++) {
                var I = limit_tickets[J];
                if (I.ticket_type == ticket_submit_order.ticket_type.student) {
                    return true
                }
            }
            return false
        ...
        function B() {
            for (var J = 0; J < limit_tickets.length; J++) {
                var I = limit_tickets[J];
                if ((ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.fc || ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.gc) && I.save_status != "" && (I.id_type == ticket_submit_order.passenger_card_type.passport || I.id_type == ticket_submit_order.passenger_card_type.work || I.id_type == ticket_submit_order.passenger_card_type.taiwan)) {
                    return true
                } else {
                    if ((ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.wc || ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.dc) && (I.id_type == ticket_submit_order.passenger_card_type.passport || I.id_type == ticket_submit_order.passenger_card_type.work || I.id_type == ticket_submit_order.passenger_card_type.taiwan)) {
                        return true
                    }
                }
            }
        ...
        function w() {
            if (ticketInfoForPassengerForm.isAsync == ticket_submit_order.request_flag.isAsync) {
                m(ticketInfoForPassengerForm.tour_flag)
            } else {
                y(ticketInfoForPassengerForm.tour_flag)
            }
        }
        function G() {
            var I = $("#qr_submit_id");
            I.unbind("click");
            I.removeClass("btn92s").addClass("btn92")
        }
        function F() {
            var I = $("#qr_submit_id");
            I.unbind("click").bind("click", p);
            I.removeClass("btn92").addClass("btn92s")
        }
        function p() {
        ...
        submitOrderClickEvent = function() {
            if (!verifyRandCode($("#randCode")[0], true)) {
                return
            }
        ...
        submitOrderClickEvent_common = function() {
            if (isDw == "Y" && $("#chooseAllDW").is(":checked")) {
                var K = $("#ticketInfo_id").find("select[id^=seatType_]");
                var J = K.eq(0).val();
                if (J != "A" && J != "F") {
        ...
        ;
        function j(J) {
            var I = dhxWins.createWindow(J + "_", 0, 0, 660, 100);
            I.attachObject(J);
            I.clearIcon();
            I.denyResize();
            I.setModal(true);
        ...
        function y(I) {
            ...
            if (I == ticket_submit_order.tour_flag.dc) {
                K = ctx + "confirmPassenger/confirmSingle";
                L.dwAll = "N";
                if ($("#chooseAllDW")[0] && $("#chooseAllDW").is(":checked")) {
                    L.dwAll = "Y"
                }
            } else {
                if (I == ticket_submit_order.tour_flag.wc) {
                    K = ctx + "confirmPassenger/confirmGo"
                } else {
                    if (I == ticket_submit_order.tour_flag.fc) {
                        K = ctx + "confirmPassenger/confirmBack";
                        L.fczk = $("#fczk").is(":checked") ? "Y" : "N"
                    } else {
                        if (I == ticket_submit_order.tour_flag.gc) {
                            K = ctx + "confirmPassenger/confirmResign"
                        } else {
                            b("订票失败!", true, "原因： 旅程形式为<span style='color:red'>" + I + "</sapn>,非法的旅程方式", true, "lose");
                            return
                        }
                    }
                }
                ...
                success: function(M) {
                    if (M.status) {
                        if (M.data.submitStatus) {
                            ...
                            b("出票失败!", false, "原因： " + M.data.errMsg + '<a  id="xg_close_win_id">点击修改</a>', false, "lose");
                        ...
                        b("订票失败!", true, "很抱歉！请关闭窗口重新预定车票", true, "lose")
                    }
                },
                error: function(M, O, N) {
                    b("订票失败!", true, "很抱歉！网络忙，请关闭窗口稍后再试。", true, "lose");
                    return
                }
            })
        }
        function m(I) {
            ...
            if (I == ticket_submit_order.tour_flag.dc) {
                K = ctx + "confirmPassenger/confirmSingleForQueue";
                L.dwAll = "N";
                if ($("#chooseAllDW")[0] && $("#chooseAllDW").is(":checked")) {
                    L.dwAll = "Y"
                }
                ...
                if (I == ticket_submit_order.tour_flag.wc) {
                    K = ctx + "confirmPassenger/confirmGoForQueue"
                } else {
                    if (I == ticket_submit_order.tour_flag.fc) {
                        K = ctx + "confirmPassenger/confirmBackForQueue";
                        L.fczk = $("#fczk").is(":checked") ? "Y" : "N"
                    } else {
                        if (I == ticket_submit_order.tour_flag.gc) {
                            K = ctx + "confirmPassenger/confirmResignForQueue"
                        } else {
                            b("订票失败!", true, "原因： 旅程形式<span style='color:red'>" + I + "</sapn>为非法的旅程方式", true, "lose");
                            return
                        }
                    }
                }
                ...
                success: function(M) {
                    if (M.status) {
                        if (!M.data.submitStatus) {
                            ...
                            var N = new OrderQueueWaitTime(I,g,D);
                            N.start()
                        }
                    } else {
                        b("订票失败!", true, "很抱歉！请关闭窗口重新预定车票", true, "lose")
        ...
        function g(I, K, J) {
            if (K <= 5) {
                b("订单已经提交，系统正在处理中，请稍等。", false, "", false, "work")
            } else {
                if (K > 30 * 60) {
                    b("订单已经提交，预计等待时间超过30分钟，请耐心等待。", false, "", false, "queue")
                } else {
                    b("订单已经提交，最新预估等待时间" + J + "，请耐心等待。", false, "", false, "queue")
                }
            }
        }
        function D(I, L, J) {
            if (L == -1) {
                var K = "";
                if (I == ticket_submit_order.tour_flag.dc) {
                    K = ctx + "confirmPassenger/resultOrderForDcQueue"
                } else {
                    if (I == ticket_submit_order.tour_flag.wc) {
                        K = ctx + "confirmPassenger/resultOrderForWcQueue"
                    } else {
                        if (I == ticket_submit_order.tour_flag.fc) {
                            K = ctx + "confirmPassenger/resultOrderForFcQueue"
                        } else {
                            if (I == ticket_submit_order.tour_flag.gc) {
                                K = ctx + "confirmPassenger/resultOrderForGcQueue"
                            }
                        }
                    }
                }
        ...
        function z(I, J) {
            if (J.name && J.card && J.tel) {
                closeWin("transforNotice_id", true);
                $("#608_check_msg").html(J.msg);
                dhtmlx.createWin({
                    winId: "608_check",
                    closeWinId: ["608_check_close", "608_check_cancel"],
                    okId: "608_check_ok",
                    okCallBack: function() {
                        $("#608_name").html(J.name);
                        $("#608_card").html(J.card);
                        $("#608_tel").html(J.tel);
                        $("#ticketInfo").html(J.ticketInfo);
                        dhtmlx.createWin({
                            winId: "608_complain",
            ...
            if (I == -1) {
                ...
                if (I == -2) {
                    if (J.errorcode == 0) {
                        b("订票失败!", true, "原因： " + J.msg, false, "lose")
                    } else {
                        b("订票失败!", true, "原因： " + J.msg, false, "lose")
                    }
                } else {
                    if (I == -3) {
                        b("哎呀,订票失败!", true, "订单已撤销", false, "lose")
                    } else {
        ...
        function x() {
            ...
            $.ajax({
                ...
                async: true,
                success: function(I) {
                    if (!I.data.submitStatus) {
                        if (I.data.isRelogin) {
                            window.location.href = ctx + "login/init?random=" + new Date().getTime()
                        } else {
                            if (I.data.isNoActive) {
                                b(I.data.errMsg, true, "", true, "warn")
                            } else {
                                b("出票失败!", true, "原因： " + I.data.errMsg, true, "warn")
                            }
                        }
                        return
                    }
                    if (I.data.smokeStr != "" && I.data.smokeStr.length > 0) {
                        $("#dialog_smoker_msg").html("根据有关部门提供的信息，<strong>" + I.data.smokeStr + "</strong>曾因违反国务院颁布的《铁路安全管理条例》规定，在动车组列车上吸烟被处罚。为了确保公共安全，请自觉遵守有关法律规定，不要在动车组列车以及其他禁烟区域吸烟，请点击“确定”继续购票。");
                        dhtmlx.createWin({
                            winId: "dialog_smoker",
        ...
        function C(I) {
            $('div[dhxbox="1"]').hide();
            var J = I.data.get608Msg;
            if (undefined != J && typeof (J) != "undefined" && "" != J) {
                if (I.data.name && I.data.card && I.data.tel) {
                    $("#608_check_msg").html(J);
                    dhtmlx.createWin({
                        winId: "608_check",
                        closeWinId: ["608_check_close", "608_check_cancel"],
                        okId: "608_check_ok",
                        okCallBack: function() {
                            $("#608_name").html(I.data.name);
                            $("#608_card").html(I.data.card);
                            $("#608_tel").html(I.data.tel);
                            $("#ticketInfo").html(I.data.ticketInfo);
                            dhtmlx.createWin({
                                winId: "608_complain",
                                closeWinId: ["608_complain_close", "608_complain_cancel"],
                                okId: "608_complain_ok",
                    ...
                    dhtmlx.alert({
                        ...
                        callback: function() {
                            ...
                            t(ticketInfoForPassengerForm.purpose_codes, I.data.isCheckOrderInfo, I.data.doneHMD)
                        }
                    })
                }
            } else {
        ...
        function r(I, J) {
            ...
            if (ticketInfoForPassengerForm.isAsync == ticket_submit_order.request_flag.isAsync && ticketInfoForPassengerForm.queryLeftTicketRequestDTO.ypInfoDetail != "") {
                $.ajax({
                    ...
                    success: function(L) {
                        if (L.status) {
                            if (L.data.isRelogin == "Y") {
                                window.location.href = ctx + "login/init?random=" + new Date().getTime()
                            }
                            var M = E(L.data.ticket, limit_tickets[0].seat_type).split(",");
                            K = "本次列车，剩余" + (limit_tickets[0].seat_type_name).split("（")[0] + "<strong>" + M[0] + "</strong>张";
                            if (M.length > 1) {
                                K += ",无座<strong>" + M[1] + "</strong>张"
                            }
                            K += "。";
                            if (L.data.op_2 == "true") {
                                K += '<font color="red">目前排队人数已经超过余票张数，请您选择其他席别或车次。</font>';
                                $("#qr_submit_id").hide()
                            } else {
                                if (L.data.countT > 0) {
                                    K += '目前排队人数<font color="red">' + L.data.countT + "</font>人，";
                                    K += "<br/>请确认以上信息是否正确，点击“确认”后，系统将为您随机分配席位。"
                                }
                            }
                            var N = $("#sy_ticket_num_id");
                            if (N != null ) {
                                N.html(K)
                            }
                            doTicketTitleShow(true);
                            renderCheckTickInfo(limit_tickets);
                            A("checkticketinfo_id");
    ...
    function f() {
        var g = $("span[id$='_check']");
        var j = true;
        for (var h = 0; h < g.length; h++) {
            if ($(g[h]).css("display") != "none") {
                j = false;
                break
            }
        }
    ...
    function a() {
        if (limit_tickets.length > init_limit_ticket_num) {
            b("最多只能购买" + init_limit_ticket_num + "张票", true, "", true, "warn");
            return false
        } else {
            if (limit_tickets.length < 1) {
                b("至少选择一位乘客", true, "", true, "warn");
                return false
            }
        }
        var l = 0;
        ...
        var o = new Array();
        var r = new Array();
        for (var m = 0; m < limit_tickets.length; m++) {
            var q = limit_tickets[m];
            r.push(q.ticket_type);
            if ((ticket_submit_order.tour_flag.wc == ticketInfoForPassengerForm.tour_flag || ticket_submit_order.tour_flag.dc == ticketInfoForPassengerForm.tour_flag) && ticket_submit_order.ticket_type.child != q.ticket_type) {
                if (jQuery.inArray(q.id_no, o) < 0) {
                    o.push(q.id_no)
                } else {
                    var n = ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(0, 4) + "年" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(4, 2) + "月" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(6, 2) + "日";
                    b("出票失败", true, "互联网售票实行实名制：证件号<span style='color:red'><i><b>" + q.id_no + "</b></i></span>只能购买" + n + "车次" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.station_train_code + "的一张车票！", true, "warn");
                    return false
                }
            } else {
                if ((ticket_submit_order.tour_flag.wc == ticketInfoForPassengerForm.tour_flag || ticket_submit_order.tour_flag.dc == ticketInfoForPassengerForm.tour_flag) && q.save_status != "" && ticket_submit_order.ticket_type.child != q.ticket_type) {
                    if (jQuery.inArray(q.id_no, o) < 0) {
                        o.push(q.id_no)
                    } else {
                        var n = ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(0, 4) + "年" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(4, 2) + "月" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(6, 2) + "日";
            ...
            if (q.save_status != "") {
                ...
                if (ticket_submit_order.tour_flag.gc == ticketInfoForPassengerForm.tour_flag) {
                    if (g.length > 0) {
                        if (jQuery.inArray(q.seat_type, g) < 0) {
                            b("出票失败", true, (CHANGETSFLAG == "Y" ? "变更到站" : "改签") + "时，必须选择相同席别", true, "warn");
                            return false
                        }
                    }
                    g.push(q.seat_type);
                    if (h.indexOf(q.seat_type) > -1) {
                        p.push(q.seat_type)
                    }
                    if (p.length > 1) {
                        b("出票失败", true, "卧铺不支持批量" + (CHANGETSFLAG == "Y" ? "变更到站" : "改签") + "，请单张" + (CHANGETSFLAG == "Y" ? "变更到站" : "改签") + "！", true, "warn");
                        return false
                    }
                }
            }
            if (q.ticket_type == ticket_submit_order.ticket_type.disability) {
                var k = id_type_code;
                if (k != ticket_submit_order.passenger_card_type.two) {
                    b("出票失败", true, "第 " + (m + 1) + " 张车票： 当前登录用户证件类型不是二代身份证，购买残疾军人（伤残警察）优待票需使用中华人民共和国居民身份证！", true, "warn");
                    return false
                } else {
                    if (q.id_type != ticket_submit_order.passenger_card_type.two) {
                        b("出票失败", true, "第 " + (m + 1) + " 张车票： 乘客证件类型不是二代身份证，购买残疾军人（伤残警察）优待票需使用中华人民共和国居民身份证！", true, "warn");
                        return false
                    }
                }
            }
            if (ticket_submit_order.tour_flag.wc == ticketInfoForPassengerForm.tour_flag || ticket_submit_order.tour_flag.dc == ticketInfoForPassengerForm.tour_flag) {
                if (q.ticket_type == ticket_submit_order.ticket_type.student) {
                    if (q.passenger_type != ticket_submit_order.passenger_type.student) {
                        b("出票失败", true, "第 " + (m + 1) + " 张车票： 乘客不是学生，请从常用联系人中选择学生购买此车票。", true, "warn");
                        return false
                    }
                }
            }
        ...
        if (ticket_submit_order.tour_flag.gc != ticketInfoForPassengerForm.tour_flag) {
            if (jQuery.inArray(ticket_submit_order.ticket_type.child, r) == 0) {
                if (jQuery.inArray(ticket_submit_order.ticket_type.adult, r) < 0) {
                    b("儿童不能单独旅行，请与成人票一同购买", true, "", true, "warn");
                    return false
                }
            }
        }
        if (ticket_submit_order.tour_flag.fc == ticketInfoForPassengerForm.tour_flag || ticket_submit_order.tour_flag.gc == ticketInfoForPassengerForm.tour_flag) {
            if (l == 0) {
                b(ticket_submit_order.tour_flag.fc == ticketInfoForPassengerForm.tour_flag ? "此次购买返程车票至少选择一位乘客" : "此次" + (CHANGETSFLAG == "Y" ? "变更到站" : "改签") + "至少选择一位乘客", true, "", true, "warn");
                return false
            }
        }
        return true
    ...
    function d() {
        if (ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.wc || ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.dc || ticketInfoForPassengerForm.tour_flag == ticket_submit_order.tour_flag.fc) {
            var j = "";
            var h = [];
            for (var k = 0; k < limit_tickets.length; k++) {
                var g = limit_tickets[k].seat_type;
                var l = limit_tickets[k].ticket_type;
                if (l == ticket_submit_order.ticket_type.child) {
                    h.push(g)
                } else {
                    j += g
                }
            ...
            for (var k = 0; k < h.length; k++) {
                if (j.indexOf(h[k]) == -1) {
                    b("订票失败!", true, "儿童不能单独购买一种席别，儿童票席别类型需与成人票席别类型一致。", true, "lose");
                    unLoadGrayBackground();
                    return false
                }
            }
...
(function() {
    ...
    $(document).ready(function() {
        if (isLimitTran == "Y") {
            dhtmlx.alert({
                title: "温馨提示",
                ok: "确定",
                text: "尊敬的旅客，当前正值暑期旅游旺季，为防止网上囤票倒票，给广大旅客创造一个公平的购票环境，自2015年6月13日起，凡通过互联网或手机购买的本次列车车票，如需办理退票、改签和变更到站等变更业务，请持乘车人身份证件原件到就近车站办理，代办时还需持代办人的身份证件原件。请您确认后购票。",
                type: "alert-info"
            })
        }
        j();
    ...
    responseDjPassengerClick = function(aC) {
        ...
        if (aC.checked) {
            ...
            if ((az.passenger_type == "2" || az.passenger_type == "3" || az.passenger_type == "4") && aH.ticket_type != "1") {
                aI = true;
                var aB = "您是要购买";
                if (normalPassenger.passenger_type == "2") {
                    aB = aB + "儿童票吗（随同成人旅行身高1.2～1.5米的儿童，应当购买儿童票；超过1.5米时应买全价票。每一成人旅客可免费携带一名身高不足1.2米的儿童，超过一名时，超过的人数应买儿童票，详见购买儿童票有关规定。如不符合相关规定，请点击“取消”。）？"
                } else {
                    if (normalPassenger.passenger_type == "3") {
                        aB = aB + "学生票吗（凭购票时所使用的有效身份证件原件和附有学生火车票优惠卡的有效学生证原件换票乘车，详见购买学生票有关规定。如不符合相关规定，请点击“取消”。）？"
                    } else {
                        if (normalPassenger.passenger_type == "4") {
        ...
        if (aB.checked) {
            ...
            if ((aJ.passenger_type == "2" || aJ.passenger_type == "3" || aJ.passenger_type == "4") && aG.ticket_type != "1") {
                ...
                if (aJ.passenger_type == "2") {
                    aA = aA + "儿童(身高1.2~1.5米)票吗？"
                } else {
                    if (aJ.passenger_type == "3") {
                        aA = aA + "学生票吗(换取纸质车票时凭附有加盖院校公章的减价优待证的学生证(小学生凭书面证明))？"
                    } else {
    ...
    ableClickSubmitButtonOrPreStepBUtton = function() {
        ...
        aA.bind("click", submitOrderClickEvent);
    ...
    disableClickSubmitButtonOrPreStepBUtton = function() {
        ...
        az.unbind("click");
        aA.unbind("click");
        aA.removeClass("btn92s").addClass("btn92")
    }
    ;
    function ah() {
