...
(function(a) {
    a.jqPagination = function(c, b) {
        ...
        d.init = function() {
            ...
            if (d.options.line_num === null ) {
                if (d.$input.data("line_num") !== undefined) {
                    d.options.max_page = d.$input.data("line_num")
                } else {
                    d.options.max_page = 6
                }
            }
        ...
        ;
        d.setLine = function(h) {
            var g = "";
            if (d.options.max_page <= d.options.line_num) {
                for (var f = 1; f < d.options.max_page + 1; f++) {
                    var e = "btn30";
                    if (f == h) {
                        e = "btn30s"
                    }
                    g += "<a href='#nogo' class='" + e + "' data-action='cur_" + f + "'>" + f + "</a>"
                }
            } else {
                if ((h + d.options.line_num - 1) <= d.options.max_page) {
                    for (var f = h; f < h + d.options.line_num - 1; f++) {
                        var e = "btn30";
                        if (f == h) {
                            e = "btn30s"
                        }
                    ...
                    g += "<span class='btn30'>...</span>";
                    g += "<a href='#nogo' class='btn30' data-action='cur_" + d.options.max_page + "'>" + d.options.max_page + "</a>"
                } else {
                    if ((h + d.options.line_num - 1) > d.options.max_page) {
                        g += "<a href='#nogo' class='btn30' data-action='cur_1'>1</a>";
                        g += "<span class='btn30'>...</span>";
                        for (var f = (d.options.max_page - d.options.line_num + 2); f < d.options.max_page + 1; f++) {
                            var e = "btn30";
                            if (f == h) {
                                e = "btn30s"
                            }
        ...
        ;
        d.initCurrentPage = function(h, f) {
            d.options.current_page = h;
            d.options.max_page = f;
            var k = d.options.link_string
              , j = parseInt(d.options.current_page, 10)
              , e = parseInt(d.options.max_page, 10);
            d.setMaxPage(e);
            if (k !== "") {
                var i = j - 1;
                ...
                d.$el.find("a.first").attr("href", k.replace("{page_number}", "1"));
                d.$el.find("a.prev, a.previous").attr("href", k.replace("{page_number}", i));
                d.$el.find("a.next").attr("href", k.replace("{page_number}", g));
                d.$el.find("a.last").attr("href", k.replace("{page_number}", e))
            }
            d.setLine(1)
        }
        ;
...
(function() {
    ...
    $(document).ready(function() {
        ...
        $("#passengerAllTable tr:last").addClass("last");
        b = $.initPage();
        $.initSearch();
        $.initCheck();
        $("#_search_butn").css("cursor", "pointer");
        $('input[type="checkbox"]').css("cursor", "pointer");
        $("#_search_name").on("keyup", function(d) {
            $._query_passenger()
        }
        );
    ...
    jQuery.extend({
        initPage: function() {
            var d = $("#pagination").jqPagination({
                link_string: ctx + "passengers/query?page={page_number}",
                current_page: 1,
                max_page: totlePage,
                paged: function(e) {
                    if (e != 0) {
                        $.pageCallback(e, pageSize)
                    }
                }
            });
            return d
        ...
        initSearch: function() {
            $("#_search_name").on("focus", function() {
                if ($(this).val() == c) {
                    $(this).val("")
                }
            }
            );
            $("#_search_name").on("blur", function() {
                if ($(this).val() == "") {
                    $(this).val(c)
                }
            }
            );
            $("#_search_butn").on("click", function() {
                $._query_passenger()
            }
            )
        },
        initCheck: function() {
            $("#_checkAll").on("click", function() {
                if ($("#_checkAll").is(":checked")) {
                    var d = $("#passengerAllTable .check");
                    $.each(d, function(g, f) {
                        var e = $(f).val().length;
                        if ($(f).val().toString().substr(e - 1) == "N") {
                            d[g]["checked"] = "checked"
                        }
                    }
                    )
        ...
        _query_passenger: function() {
            var d = new Object();
            d.pageIndex = 1;
            d.pageSize = pageSize;
            if ($("#_search_name").val() != "" && $("#_search_name").val() != c) {
                d["passengerDTO.passenger_name"] = $("#_search_name").val()
            }
            $.ajax({
                url: ctx + "passengers/query",
                type: "post",
                data: d,
                success: function(e) {
                    if (e.status) {
                        if (e.data.flag) {
                            $.renderTable(e.data.datas);
                            totlePage = e.data.pageTotal;
                            b.initCurrentPage(1, totlePage)
                        } else {
                            if (e.data.message && e.data.message != "" && e.data.message.length > 0) {
                                dhtmlx.alert({
                                    title: "加载常用联系人",
                                    ok: "确定",
                                    text: e.data.message,
        ...
        pageCallback: function(f, d, e) {
            var g = new Object();
            g.pageIndex = f;
            g.pageSize = d;
            if ($("#_search_name").val() != "" && $("#_search_name").val() != c) {
                g["passengerDTO.passenger_name"] = $("#_search_name").val()
            }
            $.ajax({
                url: ctx + "passengers/query",
                type: "post",
                ...
                success: function(h) {
                    if (h.status) {
                        if (h.data.flag) {
                            $.renderTable(h.data.datas)
                        } else {
                            if (h.data.message && h.data.message != "" && h.data.message.length > 0) {
                                dhtmlx.alert({
                                    title: "加载常用联系人",
                                    ok: "确定",
                                    text: h.data.message,
        ...
        renderTable: function(i) {
            ...
            if (i && i.length > 0) {
                ...
                for (var f = 0; f < o; f++) {
                    var d = i[f];
                    var h = "<tr>";
                    var n = d.isUserSelf && "Y" == d.isUserSelf;
                    if (f == o - 1) {
                        h = '<tr class="last">'
                    }
                    var g = "";
                    ...
                    h += "<td><input " + g + " value='" + d.passenger_name + "#" + d.passenger_id_type_code + "#" + d.passenger_id_no + "#" + d.isUserSelf + "' type='checkbox' class='check' /><label>" + (f + 1) + "</label></td>";
                    h += "<td>" + d.passenger_name + "</td>";
                    h += "<td>" + d.passenger_id_type_name + "</td>";
                    h += "<td>" + d.passenger_id_no + "</td>";
                    h += "<td>" + d.mobile_no + "</td>";
                    var l = d.passenger_type_name;
                    if (!l) {
                        l = ""
                    }
                    h += "<td>" + l + "</td>";
                    var j = d.total_times;
                    if (d.passenger_id_type_code == "2") {
                        h += "<td  style='color: red' title=\"本网站不再支持一代居民身份证，请更改为二代居民身份证.\">未通过</td>"
                    } else {
                        if (j == "95" || j == "97") {
                            h += "<td style='color: #0077FF;'>已通过</td>"
                        } else {
                            if (j == "93" || j == "99") {
                                if (d.passenger_id_type_code == "1") {
                                    h += "<td style='color: #0077FF;'>已通过</td>"
                                } else {
                                    h += "<td style='color: #0077FF;'>预通过</td>"
                                }
                            } else {
                                if (j == "94" || j == "96") {
                                    if (d.passenger_id_type_code == "1") {
                                        h += "<td  style='color: red' title=\"根据本网站的服务条款，您需要提供真实、准确的本人资料，您注册的信息与其他用户重复，请您尽快到就近的办理客运售票业务的铁路车站完成身份核验，通过后即可在网上购票，本网站将核验与您身份信息重复的用户，谢谢您的支持。\">未通过</td>"
                                    } else {
                                        h += "<td  style='color: red' title=\"身份信息核验未通过，详见《铁路互联网购票身份核验须知》。\">未通过</td>"
                                    }
                                } else {
                                    if (j == "92" || j == "98") {
                                        if (d.passenger_id_type_code == "B" || d.passenger_id_type_code == "H" || d.passenger_id_type_code == "C" || d.passenger_id_type_code == "G") {
                                            h += '<td title="身份信息未经核验，需持在本网站填写的有效身份证件原件到车站售票窗口办理预核验，详见《铁路互联网购票身份核验须知》。">请报验</td>'
                                        } else {
                                            h += '<td title="身份信息未经核验，需持二代居民身份证原件到车站售票窗口办理核验，详见《铁路互联网购票身份核验须知》。">待核验</td>'
                                        }
                                    } else {
                                        if (j == "91") {
                                            if (d.passenger_id_type_code == "B" || d.passenger_id_type_code == "H" || d.passenger_id_type_code == "C" || d.passenger_id_type_code == "G") {
                    ...
                    var e = "";
                    if (!n) {
                        e = "<a class='i-del' href='javascript:$.passenger_del(\"" + d.passenger_name + '","' + d.passenger_id_type_code + '","' + d.passenger_id_no + '","' + d.isUserSelf + "\")'></a><a class='i-edit' href='javascript:$.passenger_show(\"" + d.passenger_name + '","' + d.passenger_id_type_code + '","' + d.passenger_id_no + '","' + d.passenger_type + "\")'></a>"
                    }
                    h += "<td class='tc'>" + e + "</td></tr>";
                    m += h
                }
                $("#passengerAllTable").html(m);
                $('input[type="checkbox"]').css("cursor", "pointer")
            } else {
                $("#passengerAllTable").html("")
            }
        },
        passenger_show: function(f, g, d, e) {
            otsRedirect("post", ctx + "passengers/show", {
                passenger_name: f,
                passenger_id_type_code: g,
                passenger_id_no: d,
                passenger_type: e
            })
        },
        passenger_delChecked: function() {
            var e = $("#passengerAllTable input[class='check']:checked");
            if (e.length > 0) {
                a = null ;
                dhtmlx.createWin({
                    winId: "dialog_del",
                    closeWinId: ["dialog_del_cancel", "dialog_del_close"],
                    okId: "dialog_del_ok",
                    okCallBack: function() {
                        $.to_del()
                    }
                ...
                a = {
                    passenger_name: d.join("#") + "#",
                    passenger_id_type_code: h.join(""),
                    passenger_id_no: f.join("#") + "#",
                    isUserSelf: g.join("#")
                }
            } else {
                dhtmlx.alert({
        ...
        passenger_del: function(e, g, d, f) {
            a = null ;
            dhtmlx.createWin({
                winId: "dialog_del",
                closeWinId: ["dialog_del_cancel", "dialog_del_close"],
        ...
        passenger_addInit: function() {
            otsRedirect("post", ctx + "passengers/addInit")
        }
    })
}
)();
